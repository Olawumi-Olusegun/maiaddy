...
const DesktopNav = () => {
    ...
    return (
        <header
            className={cn(
                "hidden md:block fixed top-0 pt-5 left-0 right-0 z-[80] w-full  transition-all",
                isScrolled && "backdrop-blur supports-[backdrop-filter]:bg-background-transparent "
            )}
        >
            <div className="max-w-7xl mx-auto flex h-20 bg-white rounded-full px-5 items-center justify-between">
                        ...
                        {links.map((link, index) => (
                            ...
                            >
                                <Link
                                    to={link.href}
                                    className={cn(
                                        "text-muted font-semibold flex items-center gap-x-0.5 group-hover:text-accent transition-all px-3 py-1 rounded-md hover:bg-secondary",
                                        pathname === link.href &&
